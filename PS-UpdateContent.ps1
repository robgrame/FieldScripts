...
param(
    ...
    [parameter(Mandatory=$true, HelpMessage="Specify Logging Path")]
    [ValidateNotNullOrEmpty()]
    [string]$LoggingPath
)
Begin {
                ...
                for ($RowIndex = 0; $RowIndex -lt $DGVResults.RowCount; $RowIndex++) {
                    ...
                    if ($CellAppSelected -eq $true) {
                        ...
                        if ($Application -ne $null) {
                            ...
                            $AppDistributionStatus = Get-CMDistributionStatus -Id $Application.PackageID | Select-Object  LastUpdateDate, PackageID, SoftwareName, NumberErrors, NumberinProgress, NumberSuccess, NumberUnknown, Targeted, SourceVersion 
                            $AppDistributionStatus | Export-Csv -Path "$($loggingPath)\ApplicationDistributionStatus.csv"  -Append
                  
                            write-host $Application.SummarizationTime $Application.LocalizedDisplayName, $Application.PackageID, $AppDistributionStatus.NumberErrors, $AppDistributionStatus.NumberinProgress, $AppDistributionStatus.NumberSuccess, $AppDistributionStatus.NumberUnknown, $AppDistributionStatus.Targeted, $AppDistributionStatus.SourceVersion| Format-Table
                        ...
                        if ($Package -ne $null) {
                            ...
                            if ($Package.PkgSourcePath -ne $null) {
                                if ($Package.PkgSourcePath -match [regex]::Escape($TextBoxMatch.Text)) {
                                    ...
                                    $PkgDistributionStatus = Get-CMDistributionStatus -Id $Package.PackageID | Select-Object  LastUpdateDate, PackageID, SoftwareName, NumberErrors, NumberinProgress, NumberSuccess, NumberUnknown, Targeted, SourceVersion 
                                    $PkgDistributionStatus | Export-Csv -Path "$($loggingPath)\PkgDistributionStatus.csv"  -Append
                          
                                    write-host $Package.SourceDate $Package.LocalizedDisplayName, $Package.PackageID, $AppDistributionStatus.NumberErrors, $AppDistributionStatus.NumberinProgress, $AppDistributionStatus.NumberSuccess, $AppDistributionStatus.NumberUnknown, $AppDistributionStatus.Targeted, $AppDistributionStatus.SourceVersion| Format-Table
