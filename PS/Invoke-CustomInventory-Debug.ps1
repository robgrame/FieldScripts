...
if (@(Get-ChildItem HKLM:SOFTWARE\Microsoft\Enrollments\ -Recurse | Where-Object { $_.PSChildName -eq 'MS DM Server' })) {
	...
	# Get AzureAD Device ID and Tenant ID
	
}

Write-Output "Retrieving Intune device information"
if ($ManagedDeviceInfo -eq $null) {
	$ManagedDeviceName = "Unknown"
	$ManagedDeviceID = "Unknown"
	Write-Output "DEBUG: No Intune device information found. Setting ManagedDeviceName and ManagedDeviceID to 'Unknown'."
} else {
	$ManagedDeviceName = $ManagedDeviceInfo.EntDeviceName
	$ManagedDeviceID = $ManagedDeviceInfo.EntDMID
	Write-Output "DEBUG: Intune device information found. Setting ManagedDeviceName to '$ManagedDeviceName' and ManagedDeviceID to '$ManagedDeviceID'."	
}
...
# Debugging: Start of APPINVENTORY section
Write-Output "DEBUG: Starting APPINVENTORY section"

if ($CollectAppInventory) {
    Write-Output "DEBUG: CollectAppInventory is enabled"

    # Get SID of current interactive users
    $CurrentLoggedOnUser = (Get-CimInstance win32_computersystem).UserName
    Write-Output "DEBUG: CurrentLoggedOnUser: $CurrentLoggedOnUser"
    ...
    if (-not ([string]::IsNullOrEmpty($CurrentLoggedOnUser))) {
        ...
        Write-Output "DEBUG: UserSid: $UserSid"
    } else {
        $UserSid = $null
        Write-Output "DEBUG: No logged-on user found"
    }

    # Get Apps for system and current user
    Write-Output "DEBUG: Retrieving installed applications"
    $MyApps = Get-InstalledApplications -UserSid $UserSid
    Write-Output "DEBUG: Total applications found: $($MyApps.Count)"

    $UniqueApps = ($MyApps | Group-Object Displayname | Where-Object { $_.Count -eq 1 }).Group
    Write-Output "DEBUG: Unique applications count: $($UniqueApps.Count)"

    $DuplicatedApps = ($MyApps | Group-Object Displayname | Where-Object { $_.Count -gt 1 }).Group
    Write-Output "DEBUG: Duplicated applications count: $($DuplicatedApps.Count)"
    ...
    Write-Output "DEBUG: Newest duplicate applications count: $($NewestDuplicateApp.Count)"

    $CleanAppList = $UniqueApps + $NewestDuplicateApp | Sort-Object DisplayName
    Write-Output "DEBUG: Clean application list count: $($CleanAppList.Count)"

    $AppArray = @()
    foreach ($App in $CleanAppList) {
        Write-Output "DEBUG: Processing application: $($App.DisplayName)"
    ...
    Write-Output "DEBUG: Completed processing applications"
    $AppPayLoad = $AppArray
    Write-Output "DEBUG: AppPayLoad created with $($AppPayLoad.Count) items"
}
