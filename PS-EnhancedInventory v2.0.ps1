...
$CustomerId = "331b5606-d9cd-4ce6-84de-df26903c39ff"  

# Replace with your Primary Key
$SharedKey = "fKNU0KRtHU33D4Lp7npiLzHkKXenWMZe5t/+hqpSDfItaveVBXN6T54TnNQQb5NN+ItuwE350b2IlMfoC43PTw=="
...
Function Get-DeliveryOptimizationStats() {
    <#
  .SYNOPSIS
      Retrieve Delivery Optmization Statistiscs
  
  .DESCRIPTION
      Retrieve Delivery Optmization statistics for each file on a specific machine
  
  .NOTES
      Author:      Roberto Gramellini
      Contact:     @robgrame
      Created:     2022-01-14
      Updated:     2022-01-14
  
      Version history:
      1.0.0 - (2022-11-03) Function created
  #>


   $DOStats = Get-DeliveryOptimizationStatus | Select-Object FileId, FileSize, FileSizeInCache, Status, ExpireOn, SourceURL, DownloadMode, CacheHost, DownloadRateBytes, BytesFromHttp, BytesFromPeers, BytesFromLanPeers, BytesFromGroupPeers, BytesFromInternetPeers, BytesFromCacheServer, DownloadDuration, BytesReceived, BytesSent, BytesToGroupPeers, BytesToInternetPeers, BytesToLanPeers, HttpConnectionCount, GroupConnectionCount,InternetConnectionCount
...
$DeliveryOptimizationStats = Get-DeliveryOptimizationStats

$DOStatArray = @()



foreach ($DOEntry in $DeliveryOptimizationStats){
   $tempDOEntry = New-Object -TypeName PSObject
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "ComputerName" -Value "$ComputerName" -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "FileID" -Value $DOEntry.FileId -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "FileSize" -Value $DOEntry.FileSize -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "FileSizeInCache" -Value $DOEntry.FileSizeInCache -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "Status" -Value $DOEntry.Status -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "DownloadDuration" -Value $DOEntry.DownloadDuration -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "SourceURL" -Value $DOEntry.SourceURL -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "DownloadMode" -Value $DOEntry.DownloadMode -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "FileSizeInCache" -Value $DOEntry.FileSizeInCache -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "CacheHost" -Value $DOEntry.CacheHost -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "DownloadRateBytes" -Value $DOEntry.DownloadRateBytes -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "BytesFromHttp" -Value $DOEntry.BytesFromHttp -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "BytesFromPeers" -Value $DOEntry.BytesFromPeers -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "BytesFromLanPeers" -Value $DOEntry.BytesFromLanPeers -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "BytesFromGroupPeers" -Value $DOEntry.BytesFromGroupPeers -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "BytesFromInternetPeers" -Value $DOEntry.BytesFromInternetPeers -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "BytesFromCacheServer" -Value $DOEntry.BytesFromCacheServer -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "BytesReceived" -Value $DOEntry.BytesReceived -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "BytesSent" -Value $DOEntry.BytesSent -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "BytesToGroupPeers" -Value $DOEntry.BytesToGroupPeers -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "BytesToInternetPeers" -Value $DOEntry.BytesToInternetPeers -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "BytesToLanPeers" -Value $DOEntry.BytesToLanPeers -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "HttpConnectionCount" -Value $DOEntry.HttpConnectionCount -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "GroupConnectionCount" -Value $DOEntry.GroupConnectionCount -Force
   $tempDOEntry | Add-Member -MemberType NoteProperty -Name "InternetConnectionCount" -Value $DOEntry.InternetConnectionCount -Force
   $DOStatArray += $tempDOEntry

}



$DOPayload = $DOStatArray

$DoStatsReportJSON = $DOPayload | ConvertTo-Json
...
$ResponseDOStats = Send-LogAnalyticsData -customerId $customerId -sharedKey $sharedKey -body ([System.Text.Encoding]::UTF8.GetBytes($DoStatsReportJSON)) -logType $DOStatsLogName
...
if ($CollectDOStats) {
    if ($ResponseDOStats -match "200 :") {
        
        $OutputMessage = $OutPutMessage + " DO Statistics: OK " + $ResponseDOStats
    }
    else {
        $OutputMessage = $OutPutMessage + " DO Statistics: Fail "
    }
}
